...
import { 
  ...
  ShoppingBag, ChevronDown, ChevronUp, CreditCard, Wallet, Truck
} from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select className="w-full p-2 border rounded mt-1" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="number" step="0.01" required className="w-full p-2 border rounded mt-1" value={formData.price || ''} onChange={e => setFormData({...formData, price: parseFloat(e.target.value)})} />
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700">Stock</label>
                <input type="number" required className="w-full p-2 border rounded mt-1" value={formData.stock || ''} onChange={e => setFormData({...formData, stock: parseInt(e.target.value)})} />
            </div>
          </div>
          
          <div>
             <label className="block text-sm font-medium text-gray-700">Image URL</label>
             <input className="w-full p-2 border rounded mt-1" value={formData.imageUrl || ''} onChange={e => setFormData({...formData, imageUrl: e.target.value})} />
          </div>

          <div className="flex items-center gap-2">
             <input type="checkbox" id="rx" checked={formData.requiresPrescription || false} onChange={e => setFormData({...formData, requiresPrescription: e.target.checked})} />
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setFormData({}); setIsEditing(true); }} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-emerald-700 transition">
      ...
      <div className="bg-white p-6 rounded-lg shadow-md max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
              required 
              className="w-full p-2 border rounded mt-1 focus:ring-emerald-500 focus:border-emerald-500" 
              value={formData.name || ''} 
          ...
          <div>
            ...
            <input 
              className="w-full p-2 border rounded mt-1 text-gray-500 bg-gray-50" 
              value={formData.slug || ''} 
...
export const AdminStores = () => {
    ...
    if (isEditing) {
        return (
            <div className="bg-white p-6 rounded-lg shadow-md max-w-2xl mx-auto">
                <h3 className="text-xl font-bold mb-4">Manage Store</h3>
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div><label className="block text-sm">Store Name</label><input required className="w-full border p-2 rounded" value={formData.name||''} onChange={e=>setFormData({...formData, name: e.target.value})}/></div>
                    <div><label className="block text-sm">Address</label><input required className="w-full border p-2 rounded" value={formData.address||''} onChange={e=>setFormData({...formData, address: e.target.value})}/></div>
                    <div className="grid grid-cols-2 gap-4">
                        <div><label className="block text-sm">Latitude</label><input type="number" step="any" required className="w-full border p-2 rounded" value={formData.lat||''} onChange={e=>setFormData({...formData, lat: parseFloat(e.target.value)})}/></div>
                        <div><label className="block text-sm">Longitude</label><input type="number" step="any" required className="w-full border p-2 rounded" value={formData.lng||''} onChange={e=>setFormData({...formData, lng: parseFloat(e.target.value)})}/></div>
                    </div>
                    ...
                    <div className="flex justify-end gap-2 pt-4">
                        <button type="button" onClick={()=>setIsEditing(false)} className="px-4 py-2 text-gray-600 hover:bg-gray-100 rounded">Cancel</button>
                        <button type="submit" className="px-4 py-2 bg-emerald-600 text-white rounded hover:bg-emerald-700">Save Store</button>
                    </div>
                </form>
            </div>
        )
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => { setFormData({}); setIsEditing(true); }} className="bg-emerald-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:bg-emerald-700">
                    <Plus className="w-4 h-4" /> Add Store
                </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {stores.map(store => (
                    <div key={store.id} className="bg-white p-4 rounded-lg shadow border border-gray-200 flex justify-between items-start">
                        <div>
                            <h3 className="font-bold text-gray-800 flex items-center gap-2"><MapPin className="w-4 h-4 text-emerald-600"/> {store.name}</h3>
                            <p className="text-sm text-gray-600 mt-1">{store.address}</p>
                            <p className="text-xs text-gray-500 mt-1">{store.phone}</p>
                            <p className="text-xs text-gray-400 mt-2 font-mono">Lat: {store.lat}, Lng: {store.lng}</p>
                        </div>
...
export const AdminDashboard = () => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <StatCard title="Active Stores" value={stores.length} icon={MapPin} color="bg-emerald-500 text-emerald-500" />
        </div>
    );
};
            ...
            <div className="bg-white p-6 rounded-lg shadow">
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <input className="w-full p-2 border rounded mt-1" value={localSettings.siteName} onChange={e => setLocalSettings({...localSettings, siteName: e.target.value})} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-gray-700">Logo URL</label>
                        <div className="flex gap-4 items-center">
                            <input className="flex-1 p-2 border rounded mt-1" value={localSettings.logoUrl} onChange={e => setLocalSettings({...localSettings, logoUrl: e.target.value})} />
                            <img src={localSettings.logoUrl} alt="Preview" className="w-10 h-10 object-contain border rounded bg-gray-50" />
                        </div>
                    </div>
                    <button onClick={handleSave} className="bg-emerald-600 text-white px-4 py-2 rounded hover:bg-emerald-700 flex items-center gap-2">
                    ...
                    {settings.paymentMethods.map(pm => (
                        <div key={pm.id} className="flex items-center justify-between p-4 border rounded-lg bg-gray-50">
                            <div className="flex items-center gap-4">
                                <div className={`p-2 rounded-full ${pm.enabled ? 'bg-emerald-100 text-emerald-600' : 'bg-gray-200 text-gray-500'}`}>
